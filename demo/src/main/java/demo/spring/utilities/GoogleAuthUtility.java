...
public class GoogleAuthUtility {
    ...
    public GoogleAuthUtility(){
        googleAuthenticator = new GoogleAuthenticator();
        googleAuthenticator.setCredentialRepository(new CredentialRepository());
    }

    public String generateQRCode(String key, String username, String issuer) throws WriterException, IOException {
        ...
        BitMatrix matrix = qrCodeWriter.encode(otpAuthUrl, BarcodeFormat.QR_CODE, 200, 200);
        var stream = new ByteArrayOutputStream();
        MatrixToImageWriter.writeToStream(matrix, "png", stream);
        return new String(Base64.getEncoder().encode(stream.toByteArray()));
    }

    public String generateBase32Encoding(String key) {
        String hash;
        try {
